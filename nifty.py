...
lotSize           = 75
pnl_log_file      = "15thSept.csv"    #file path for logs
...
last_minute       = None           #to stop the re-entry in the same minute 
...
def getNiftyATM():
    try:
        ...
        close_value = float(make_request('NSE', '26000'))
        
        # round off to get the atm [NIFTY - strike difference is 50, and that's why we round it off to 50] 
        return round(close_value / 50) * 50
    except Exception as e:
        print(f"Error while getting NIFTY ATM: {e}")
        return None

def gettingInstruments(targetStrike, strikeRange, index, exchange='NSE_FO'):
    ...
    try:
        ...
        filteredDf['instrument_key'] = filteredDf['instrument_key'].str.replace("NSE_FO", "NFO")
        ...
        if index == 'NIFTY':
            ...
            index_token = str(symboldf[symboldf['instrument_key'] == 'NSE_INDEX|Nifty 50']['exchange_token'].iloc[0])
            print(index_token)
        return filteredDf['instrument_key'].tolist() + [f'NSE|{index_token}'], callOptions, putOptions
...
def place_position(option_df, option_type, strike_input):
    ...
    try:
        if strike_input == "ATM":
            ...
            strike_offset = -50 * distance if option_type == "CE" else 50 * distance
    ...
    try:
        ...
        entry_price = float(make_request("NFO", str(token)))
...
def check_and_exit_positions(message):
    ...
    try:
        ...
        try:
            ...
            ltp = float(make_request("NFO", str(token)))
...
def main():
    ...
    instrumentKeys, callOptions, putOptions = gettingInstruments(atm, strikeRange=500, index='NIFTY', exchange='NSE_FO')
